...
class Item_Group extends CI_Controller {
	...
	public function index($group_id=0)
	{
		...
		if($group_id!=0)
		{
			$group_data['group_detail']	=	$this->group_model->get_group_by_id($group_id);
		}else{
			$group_data['group_detail']	=	NULL;
		}

		$data['navigation']			=	$this->load->view('template/navigation',$nav_data,TRUE);
		$data['footer']				=	$this->load->view('template/footer','',TRUE);
		$data['content']			=	$this->load->view('pages/asset_mgt/item_group',$group_data,TRUE);
